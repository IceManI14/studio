
export const COMPETITOR_DETAILS: Record<string, { title?: string; details: string[] }> = {
  "Culligan-Quench": {
    details: [
      "Filtration only",
      "Terrible Service",
      "Offer R/O",
      "Pricing: $45-$80 for Hot/Cold (qty dependent)",
      "Offer Ice and water units",
      "Owns BEVI ($350/month per unit)",
      "Acquired: Waterlogic, Blue Reserve, Eastern Pure, Stonybrook",
      "Similar contracts to Optimum",
      "Coolers: Wellsys 9000, Wellsys 11000, W9, Wellsys 12000, I14, I15, I16, Quench Brand (Q3, Q4, etc.), Waterlogic. Note: Ion 902, 200, 400 (from Stonybrook, not putting new in field)"
    ]
  },
  "Atlantic Pure": {
    title: "Mostly Rhode Island and Southern MA",
    details: [
      "Wellsys dealer",
      "Brand new company",
      "Sometimes Ultra Filtration instead of RO in W9",
      "Pricing: $49-$69 per unit",
      "Offer Ice and water",
      "Contracts: Same as Optimum, use Pure Water",
      "Coolers: W9, i14, i15, i16"
    ]
  },
  "Boston Bean": {
    title: "Coffee Company",
    details: [
      "Filtration only",
      "NO RO",
      "Pricing: $40-$80 (qty dependent)",
      "Offers BEVI",
      "Coolers: Ion 200, Ion 400, Alpine"
    ]
  },
  "Ready Refresh/Primo": {
    details: [
      "5 Gallon Bottles and Filtration",
      "Bottled Brands: Nestle Pure Life, Poland Spring, Crystal Rock, Primo, Vermont Pure",
      "Filtration Line: Accupure",
      "Terrible service",
      "Rarely offer RO",
      "Cleaning Fee: $95 (recommended every 3 months, often charged unknowingly)",
      "Filtration Cost: $40-$50",
      "Bottle Pricing: $10-$15 (State contract: $4.75-$5.00/bottle + $10 delivery)",
      "Coolers: Oasis (filter cooler). Bottles also offered with various brands."
    ]
  },
  "WB Mason": {
    title: "Paper company that offers water",
    details: [
      "5 gallons and filtration",
      "Bottled Brands: Blizzard water or Poland Spring",
      "Bottle Price: $5-$13",
      "Filtration Pricing: $25-$50",
      "Don’t offer RO",
      "Coolers: Oasis (filter cooler). Bottles also offered."
    ]
  },
  "Cintas": {
    title: "Uniform company that offers water",
    details: [
      "Filtration only",
      "Pricing: $35-$60",
      "Don’t offer RO",
      "Coolers: Waterlogic, Oasis"
    ]
  },
  "Aramark": {
    details: [
      "Filtration only",
      "Pricing: $40-$80",
      "Charge for Filter changes (cost unknown)",
      "Don’t Offer RO",
      "Coolers: Alpine"
    ]
  },
  "Blue Drop": {
    details: ["Coolers: Brio"]
  },
  "Cleartide": {
    details: ["Coolers: W9, i14, i15, i16"]
  },
  "Crown Coffee": {
    details: ["Coolers: Ion 200, Ion 400"]
  },
  "ELKAY wall uni!": {
    title: "Wall Mounted Units",
    details: [
      "Often found in schools, gyms, and public facilities.",
      "Typically provides only chilled water.",
      "May have bottle filler functionality.",
      "Service and maintenance can vary widely."
    ]
  },
  "Other": {
    details: ["Details for this competitor are not pre-defined. Add specific notes if available."]
  }
};
