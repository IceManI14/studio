...
'use server';
/**
 * @fileOverview AI flow to suggest a company name based on GPS coordinates.
 *
...
const mockReverseGeocodeTool = ai.defineTool(
  {
    ...
    outputSchema: z.object({
      locationDescription: z.string().describe('A textual description of the location, potentially including address and nearby points of interest or company names.'),
    }),
  },
  async ({ latitude, longitude }) => {
    // Simulate different responses based on coordinates for variety
    if (latitude > 40 && longitude < -100) { // e.g., West USA
      return { locationDescription: `Area around 123 Innovation Drive, Tech City, CA. Nearby: "Future Systems Inc.", "Cafe Bytes"` };
    } else if (latitude < 30 && longitude > -90) { // e.g., Southeast USA
      return { locationDescription: `Vicinity of 456 Commerce St, Business Hub, FL. Known establishments: "Ocean Breeze Logistics"` };
    } else { // Generic
      return { locationDescription: `Location at coordinates ${latitude.toFixed(4)}, ${longitude.toFixed(4)}. Potential business: "Local Services Co." or residential area.`};
    }
  }
);
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert at identifying company names from location descriptions.
Given the following location information: {{{locationDescription}}}

Extract the most likely primary company name operating at this location.
