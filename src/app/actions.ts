...
export async function saveVisitAction(payload: SaveVisitPayload): Promise<{ visit?: Visit; error?: string; isNewVisit?: boolean }> {
  ...
  try {
    ...
    const visitForDb: Omit<Visit, 'id' | 'dealClosed'> = {
      ...
      businessCardImageFrontUrl: (payload.hasBusinessCard || false) ? payload.businessCardImageFrontUrl ?? null : null,
      businessCardImageBackUrl: (payload.hasBusinessCard || false) ? payload.businessCardImageBackUrl ?? null : null,
